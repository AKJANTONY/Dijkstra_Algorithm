d3.dijkstra = function () {
    ...
    dijkstra.run = function (src) {
        ...
        // reset styles
        d3.selectAll('.node').style('fill', '#fff').attr('r', 10).style('stroke-width', '1');
        d3.select('.'+src.name).style('fill', '#fdb03f').style('stroke-width', '0');
        d3.selectAll('.nodetext').text(function(d){ return d.name});
        d3.selectAll('.linktext').style('fill', '#fdb03f');//style('opacity', '0');
        ...
        function tick() {
            ...
            d3.select('.'+current.name)
                .transition().delay(translate_speed*2/5).duration(translate_speed/5).attr('r', 15)
